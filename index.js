...
// Email setup - using Resend (works with Render free tier)
...
app.post('/api/auth/send-login-email', async (req, res) => {
  try {
    ...
    if (emailLower === OWNER_EMAIL) {
      ...
      await sendEmail(emailLower, '🔐 Your FBM Ops Hub Login Link (Owner)', `<p>Click below to login as <strong>Owner</strong> (link expires in 15 minutes):</p>
    ...
    await sendEmail(emailLower, `🔐 Your FBM Ops Hub Login Link (${user.role})`, `<p>Click below to login as <strong>${user.role.toUpperCase()}</strong> (link expires in 15 minutes):</p>
...
app.post('/api/auth/signup', async (req, res) => {
  try {
    ...
    await sendEmail(process.env.EMAIL_ADMIN_RECIPIENTS || OWNER_EMAIL, `👤 New User Signup Request: ${name}`, `
...
app.get('/api/auth/approve', async (req, res) => {
  try {
    ...
    await sendEmail(email, '✅ Your FBM Ops Hub Account Approved!', `
...
app.get('/api/auth/reject', async (req, res) => {
  try {
    ...
    await sendEmail(email, '❌ Your FBM Ops Hub Signup Request', `<p>Your signup request has been rejected. Contact the administrator if you have questions.</p>`);
